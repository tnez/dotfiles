...
1. Run `git diff main...HEAD` and `git log main..HEAD --oneline` to understand
   all changes since main.
2. Push the current branch to origin with `-u`.
3. Draft a short PR title under 70 characters and a concise body:
   - `## Summary`: 1-3 succinct bullet points.
   - `## Test plan`: bulleted checklist.
4. Create the PR with `gh pr create --base main`. Try with `--draft` first. If
